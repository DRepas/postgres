...
insert into test_multirange_join_2 select int4multirange(int4range(g, g+10),int4range(g+20, g+30),int4range(g+40, g+50)) from generate_series(1,200) g;
insert into test_multirange_join_2 select '{}'::int4multirange from generate_series(1,50) g;
insert into test_multirange_join_2 select int4multirange(int4range(g, g+10000)) from generate_series(1,100) g;
...
insert into test_range_join select int4range(g, g+10) from generate_series(1,200) g;
insert into test_range_join select int4range(g, g+10000) from generate_series(1,100) g;
...
insert into test_range_join select NULL from generate_series(1,50) g;
...
CREATE TEMPORARY TABLE multi_multi_count AS
    SELECT 
        (SELECT COUNT(*) FROM test_multirange_join_1) * 
        (SELECT COUNT(*) FROM test_multirange_join_2) AS total_product;

CREATE TEMPORARY TABLE range_multi_count AS
    SELECT 
        (SELECT COUNT(*) FROM test_multirange_join_1) * 
        (SELECT COUNT(*) FROM test_range_join) AS total_product;
        
CREATE TEMPORARY TABLE elem_multi_count AS
    SELECT 
        (SELECT COUNT(*) FROM test_multirange_join_1) * 
...
JOIN multi_multi_count ON true;
...
JOIN range_multi_count ON true;
...
-- slightly lower estimation accuracy for &< and &> operands
SELECT ABS(CAST(estimated AS float) - actual) / total_product < 0.05 AS ok
FROM check_estimated_rows(
  'select * from test_multirange_join_1, test_multirange_join_2 where mr1 &< mr2')
JOIN multi_multi_count ON true;
...
-- lower estimation accuracy for <@ and @> operands
SELECT ABS(CAST(estimated AS float) - actual) / total_product < 0.25 AS ok
FROM check_estimated_rows(
  'select * from test_elem_join, test_multirange_join_1 where elem <@ mr1')
JOIN elem_multi_count ON true;
...
drop table multi_multi_count;
drop table range_multi_count;
drop table elem_multi_count;
